...
- PostgreSQL arrêté (drill réel exécuté) : /health reste 200, les routes DB répondent 500 ; restauration → **reprise automatique sans redémarrage** (200 sur la première requête, login 200) — postgres-js reconnecte par requête. Vérifier outbox (reprise après lease) en cas d'interruption pendant un événement.
- Redis arrêté (drill réel exécuté) : le process API reste vivant (/health 200), les workers BullMQ patientent, mais les routes authentifiées répondent 500 (ioredis ETIMEDOUT sur le contrôle jti/blacklist) — posture fail-closed corrigée : contrôle de blacklist JWT fail-open par défaut (AUTH_REDIS_BLACKLIST_FAIL_OPEN=true) avec timeout de commande de 1 s, stockage throttle avec échec rapide hors ligne, repli mémoire et gestion des erreurs de connexion (plus de 500 ni de crash process). Drill rejoué : Redis coupé → routes authentifiées 200 en ~1 s, /health 200, API vivante ; reprise 200 après docker start + restart API. Les tests unitaires simulent déjà Redis indisponible (jwt blacklist, token cleanup).
...
## Correctifs appliqu�s (2026-08-11)
- Attachements publics : le guard et les services v�rifient d�sormais la cl� ttachments (et le repli publicAttachments), fin du 404 � Fonction indisponible �.
- Upload/t�l�chargement/statut des pi�ces jointes couverts c�t� BFF (oute-policy) et client (tickets + conversations).
- Escalade automatique : les tickets ASSIGNED en retard sans activit� sont d�sassign�s puis r�-achemin�s (borne 10/passage, historique syst�me).
- Alertes SLA : les notifications de violation sont relanc�es toutes les 6 h tant que le ticket reste ouvert et en retard (m�tadonn�es JSONB), plus de coupure apr�s la date �chue.
- Dashboard : tRisk/overdue/workload.slaAtRiskCount couvrent les tickets �chus ; overdue expos� ; esolvedCount ajout� � la r�solution moyenne.
- Cat�gories : r�les multiples (	argetRoles) support�s par le moteur d'assignation (repli 	argetRole), migration 0017.
- D�partements : algorithme (LEAST_LOADED/ROUND_ROBIN), auto-assignation et charge max configurables via DTO et UI admin.
- Utilisateurs : pause (isAvailable) et absence (bsenceEndsAt) g�r�es dans l'assignation manuelle et expos�es � l'admin.
- Login CSRF : repli same-origin en d�veloppement quel que soit le port local (fin du 403 sur 3001).
