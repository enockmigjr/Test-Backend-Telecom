...
- État : cadrage validé par l'utilisateur (11 points), correctifs bloquants livrés (commentaires 500, modals intégrations, responsivité bot/portail), statistiques support public livrées, performance agents livrée, Makefile de base livré, pondérations de charge admin livrées. Phases restantes : évaluation (score/page dédiée), pause/absence agents, dashboards (agent + supervision), réponse au demandeur + satisfaction, config admin étendue, déploiement production, Keycloak SSO + Keycloakify.
