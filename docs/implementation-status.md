...
| Composant | Notes |
| --- | --- |
| Auth SSO | Keycloak unique (OIDC PKCE, RS256/JWKS), `GET /auth/me` ; ancien auth local supprimé (login/refresh/logout/logout-all/change-password) |
...
| Swagger | 28 tags, 139 opérations OpenAPI (115 chemins) ; contrat public projeté `openapi.public.json` (33 opérations), tests de non-fuite |
...
| SSO Keycloak | Actif : login/déconnexion SSO, logout toutes-sessions via API admin, console de compte ; ancien auth supprimé le 13/08/2026 |
| Tests | 90 fichiers spec (585 tests / 90 suites, vérifiés le 13/08/2026) + 24 fichiers E2E/intégration |

## Verdict

Le backend est complet et structuré (25 modules, 31 tables, 139 opérations OpenAPI, 8 files/workers). La priorité n'est plus d'ajouter des briques mais de finir les chantiers engagés (phase 09 support public, validation de la rétention) et de maintenir les contrats et la documentation à jour.
