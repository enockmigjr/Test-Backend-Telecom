...
- **Cloisonnement Départemental strict** : Un utilisateur d'un département donné (ex: _Billing_) ne peut visualiser ou modifier que les tickets rattachés à son département d'affectation (`departmentId`), sauf s'il possède le rôle global `ADMINISTRATOR`.
