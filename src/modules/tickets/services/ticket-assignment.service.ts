...
export class TicketAssignmentService {
  ...
  async update(id: string, dto: UpdateTicketInput, user: JwtPayload) {
    ...
    if (dto.priority !== undefined || dto.categoryId !== undefined) {
      ...
      if (policy) {
        const now = new Date();
        const cal = priority === 'CRITICAL' || priority === 'HIGH' ? '24_7' : 'BUSINESS_HOURS';
        const bh = await this.settingsService.getBusinessHours();
        const bd = await this.settingsService.getBusinessDays();
        updateData['slaPolicyId'] = policy.id;
        updateData['firstResponseDueAt'] = calculateSlaDueDate(
          now,
          policy.firstResponseMinutes,
          cal as '24_7' | 'BUSINESS_HOURS',
          bh,
          bd,
        );
