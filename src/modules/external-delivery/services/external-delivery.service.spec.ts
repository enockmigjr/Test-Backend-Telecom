...
describe('ExternalDeliveryService (rejeu après panne)', () => {
  ...
  it('rejoue les livraisons FAILED en les repassant PENDING puis en ré-ajoutant un job sans jobId', async () => {
    ...
    expect(queueAdd).toHaveBeenCalledWith('dispatch-outbox-event', { outboxEventId: 'outbox-1' });
    expect(queueAdd).toHaveBeenCalledWith('dispatch-outbox-event', { outboxEventId: 'outbox-2' });
  });
});
